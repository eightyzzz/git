...
static bool st7789_async_pending;	/* 异步(LVGL双缓冲)传输进行中 */
static void (*st7789_flush_done_cb)(void);
...
static void st7789_spi_init(void)
{
	...
	SPI_InitStruct.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;	//21MHz, 全屏刷新翻倍 (ST7789 标称15.5MHz, 超规但短走线通常可跑; 花屏退回 _4)
...
/* 启动一次 DMA 传输(半字计数); 不等待完成 */
static void st7789_dma_start(uint8_t *data, uint32_t halfwords, bool singlecolor)
{
	if (singlecolor) DMA1_Stream4->CR &= ~DMA_SxCR_MINC;	//MINC = 0,内存地址不自增
	else			 DMA1_Stream4->CR |= DMA_SxCR_MINC;	//MINC = 1,内存地址自增
	DMA1_Stream4->M0AR = (uint32_t)data;
	DMA1_Stream4->NDTR = halfwords;
...
static void st7789_write_gram(uint8_t data[], uint32_t length, bool singlecolor)
{
	...
	do
	{
		...
		st7789_dma_start(data, chunk_size, singlecolor);
		
		xSemaphoreTake(write_gram_semaphore, portMAX_DELAY);	//等待信号到来
...
{
	st7789_flush_done_cb = cb;
}

/* 异步 flush: 启动 DMA 后立即返回, DMA 完成(含最后一位移出)后回调 flush_done_cb。
 * 供 LVGL 双缓冲使用, 渲染下一块时本块同时在传输。 */
void st7789_flush_async(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t *colors)
{
	...
	GPIO_ResetBits(CS_PORT, CS_PIN);
	GPIO_SetBits(DC_PORT, DC_PIN);
	st7789_dma_start((uint8_t *)colors, pixels, false);
	st7789_async_pending = true;
}
...
void DMA1_Stream4_IRQHandler(void)
{
	...
	{
		if (st7789_async_pending)
		{
			/* 异步路径: 等最后一位移出(约1us)再拉高CS, 然后通知LVGL本块已可复用 */
			st7789_async_pending = false;
			while (SPI_GetFlagStatus(SPI2, SPI_FLAG_BSY) == SET);
			GPIO_SetBits(CS_PORT, CS_PIN);
			if (st7789_flush_done_cb)
				st7789_flush_done_cb();
		}
		else
		{
			/* 同步路径(整屏填充/初始化): 用信号量唤醒等待者 */
